...
from pyjob.utils import (create_args_from_config, create_template,
                         get_scheduler_infos, load_config, make_dirs,
                         parse_template, show_submission)
...
def launch_jobs(scheduler_infos, template, list_dict_args, config, submit):
    ...
    for i, dict_args in enumerate(list_dict_args):
        template_file = os.path.join(
            template_dir, "{}_{}.{}".format(jobs_name, i, job_extension)
        )
    ...
    print(
        "You can check the scripts in {}/{}_*.{}".format(
            template_dir, jobs_name, job_extension
        )
    )
    if submit:
